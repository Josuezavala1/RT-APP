...
function updateBoundaries(attenuationFactor) {
  ...
  const beamOnMinutesPerHour = n(fields.beamOnMinutesPerHour.value) ?? 6;
  ui.boundaryExposure1ft.textContent = exposure1ft === null ? "—" : `${formatNumber(exposure1ft, 1)} mR/hr`;

  if (!exposure1ft || !beamOnMinutesPerHour) {
    ui.boundary100mr.textContent = "—";
    ui.allowedRateDuringShot.textContent = "—";
    ui.boundary2mr.textContent = "—";
    return { exposure1ft: null, d100: null, d2: null, beamOnMinutesPerHour, allowedRateDuringShot: null };
  }

  // Time-weighted occupancy method for 2 mR/hr barricade only.
  const allowedRateDuringShot = 2 * (60 / beamOnMinutesPerHour);
  ...
  ui.allowedRateDuringShot.textContent = `${formatNumber(allowedRateDuringShot, 2)} mR/hr`;
  ui.boundary2mr.textContent = `${formatNumber(d2, 2)} ft`;

  return { exposure1ft, d100, d2, beamOnMinutesPerHour, allowedRateDuringShot };
}
...
function evaluateShot(shotEl) {
  ...
  const actualBallDiameter = n(shotEl.querySelector(".actual-ball").value) ?? 1;
  const measuredBallOnFilm = n(shotEl.querySelector(".measured-ball").value);

  const spdMinUgEl = shotEl.querySelector(".spd-min-ug");
  const spdMinBlowupEl = shotEl.querySelector(".spd-min-blowup");
  const spdMinFinalEl = shotEl.querySelector(".spd-min-final");
  ...
  const blowupStatusEl = shotEl.querySelector(".blowup-status");
  const filmBlowupEl = shotEl.querySelector(".film-blowup");
  const filmStatusEl = shotEl.querySelector(".film-status");
  const overallStatusEl = shotEl.querySelector(".overall-status");
  ...
  const BLOWUP_MAX_PERCENT = 20;
  const M_MAX = 1 + BLOWUP_MAX_PERCENT / 100;

  if (!d || !ugLimit) {
    helperEl.textContent = "Set d and Ug_limit to enable calculations.";
    [spdMinUgEl, spdMinBlowupEl, spdMinFinalEl, ugEl, ugStatusEl, magEl, blowupStatusEl, filmBlowupEl, filmStatusEl, overallStatusEl, exposureEl].forEach((el) => (el.textContent = "—"));
    return { warnings: ["Missing d or Ug_limit."], data: null };
  }

  const spdMinUg = pdd ? (d * pdd) / ugLimit : null;
  const spdMinBlowup = pdd ? (M_MAX * pdd) / (M_MAX - 1) : null;
  const spdMinFinal = (spdMinUg !== null && spdMinBlowup !== null) ? Math.max(spdMinUg, spdMinBlowup) : null;

  spdMinUgEl.textContent = spdMinUg === null ? "—" : `${formatNumber(spdMinUg, 2)} in`;
  spdMinBlowupEl.textContent = spdMinBlowup === null ? "—" : `${formatNumber(spdMinBlowup, 2)} in`;
  spdMinFinalEl.textContent = spdMinFinal === null ? "—" : `${formatNumber(spdMinFinal, 2)} in`;

  let ug = null;
  let ugPass = null;
  let blowupPercent = null;
  let blowupPass = null;
  let filmBlowupPercent = null;
  let filmPass = null;
  ...
  if (!spd && !pdd) {
    ...
    magEl.textContent = "—";
    blowupStatusEl.textContent = "—";
    overallStatusEl.textContent = "—";
  } else if (spd && pdd) {
    helperEl.textContent = "UG + blowup validation active.";
    ...
    if (sod > 0) {
      const m = spd / sod;
      blowupPercent = (m - 1) * 100;
      blowupPass = blowupPercent <= BLOWUP_MAX_PERCENT;
      magEl.textContent = `${formatNumber(blowupPercent, 1)}%`;
      blowupStatusEl.textContent = blowupPass ? "PASS" : "FAIL";
    } else {
      magEl.textContent = "—";
      blowupStatusEl.textContent = "FAIL";
      blowupPass = false;
      const msg = "FAIL: SPD must be greater than PDD for geometry magnification.";
      shotWarningsEl.appendChild(createWarning(msg, "red"));
      warnings.push(msg);
    }

    if (measuredBallOnFilm && actualBallDiameter) {
      const mFilm = measuredBallOnFilm / actualBallDiameter;
      filmBlowupPercent = (mFilm - 1) * 100;
      filmPass = filmBlowupPercent <= BLOWUP_MAX_PERCENT;
      filmBlowupEl.textContent = `${formatNumber(filmBlowupPercent, 1)}%`;
      filmStatusEl.textContent = filmPass ? "PASS" : "FAIL";
    } else {
      filmBlowupEl.textContent = "—";
      filmStatusEl.textContent = "PENDING";
    }

    overallPass = !!(ugPass && blowupPass);
    overallStatusEl.textContent = overallPass ? "PASS" : "FAIL";

    if (!ugPass || !blowupPass) {
      const failParts = [];
      if (!ugPass) failParts.push("UG");
      if (!blowupPass) failParts.push("Geometry Blowup");
      const msg = `FAIL: ${failParts.join(" + ")} out of spec. Minimum SPD required (UG + Blowup 20%): ${spdMinFinal === null ? "-" : `${formatNumber(spdMinFinal, 2)} in`}.`;
      shotWarningsEl.appendChild(createWarning(msg, "red"));
      warnings.push(msg);
    } else {
      shotWarningsEl.appendChild(createWarning("PASS: UG and geometry blowup are within limits.", "yellow"));
    }
  } else {
    helperEl.textContent = "Enter both PDD and SPD to compute actual UG and blowup pass/fail.";
    ugEl.textContent = "—";
    ugStatusEl.textContent = "PENDING";
    magEl.textContent = "—";
    blowupStatusEl.textContent = "PENDING";
    overallStatusEl.textContent = "PENDING";
  }

  if (!(measuredBallOnFilm && actualBallDiameter)) {
    filmBlowupEl.textContent = filmBlowupEl.textContent || "—";
    filmStatusEl.textContent = filmStatusEl.textContent || "PENDING";
  }
  ...
  return {
    ...
    data: {
      ...
      ug,
      ugPass,
      blowupMaxPercent: BLOWUP_MAX_PERCENT,
      blowupPercent,
      blowupPass,
      actualBallDiameter,
      measuredBallOnFilm,
...
function addShot() {
  shotCounter += 1;
  const shotNumber = shotCounter;
  const fragment = shotTemplate.content.cloneNode(true);
  const shotEl = fragment.querySelector(".shot-card");
  shotEl.querySelector(".shot-name").textContent = `Shot ${shotNumber} — Config 1`;
  ...
  const syncHeader = () => {
    shotEl.querySelector(".shot-name").textContent = `Shot ${shotNumber} — Config ${configInput.value}`;
  };
  syncHeader();

  [spdInput, pddInput, ugLimitInput, configInput, viewInput, directionInput, shotEl.querySelector(".actual-ball"), shotEl.querySelector(".measured-ball")].forEach((input) => {
    input.addEventListener("input", () => {
      syncHeader();
      evaluateAllShots();
    });
...
function buildPdfLines(summary) {
  ...
  lines.push(`BeamOnMinutesPerHour: ${boundaries.beamOnMinutesPerHour === null ? "-" : formatNumber(boundaries.beamOnMinutesPerHour, 1)}`);
  lines.push(`AllowedRateDuringShot_mRhr: ${boundaries.allowedRateDuringShot === null ? "-" : `${formatNumber(boundaries.allowedRateDuringShot, 2)} mR/hr`}`);
  lines.push(`2 mR/hr (Time-Weighted) distance: ${boundaries.d2 === null ? "-" : `${formatNumber(boundaries.d2, 2)} ft`}`);
  lines.push("");
  ...
  shotResults.forEach((result, idx) => {
    ...
    lines.push(`  BlowupMaxPercent: ${formatNumber(result.data.blowupMaxPercent, 0)}%`);
    lines.push(`  SPD_min_UG: ${result.data.spdMinUg === null ? "-" : `${formatNumber(result.data.spdMinUg, 2)} in`}`);
    lines.push(`  SPD_min_Blowup: ${result.data.spdMinBlowup === null ? "-" : `${formatNumber(result.data.spdMinBlowup, 2)} in`}`);
    if (result.data.overallPass === false) {
      lines.push(`  SPD_min_FINAL: ${result.data.spdMinFinal === null ? "-" : `${formatNumber(result.data.spdMinFinal, 2)} in`}`);
    }
    lines.push(`  UG: ${result.data.ug === null ? "-" : formatNumber(result.data.ug, 4)}`);
    lines.push(`  UG PASS/FAIL: ${result.data.ugPass === null ? "PENDING" : result.data.ugPass ? "PASS" : "FAIL"}`);
    lines.push(`  Geometry Blowup%: ${result.data.blowupPercent === null ? "-" : `${formatNumber(result.data.blowupPercent, 1)}%`}`);
    lines.push(`  Geometry Blowup PASS/FAIL: ${result.data.blowupPass === null ? "PENDING" : result.data.blowupPass ? "PASS" : "FAIL"}`);
    if (result.data.measuredBallOnFilm !== null) {
      lines.push(`  Film Blowup%: ${result.data.filmBlowupPercent === null ? "-" : `${formatNumber(result.data.filmBlowupPercent, 1)}%`}`);
      lines.push(`  Film Blowup PASS/FAIL: ${result.data.filmPass === null ? "PENDING" : result.data.filmPass ? "PASS" : "FAIL"}`);
    }
    lines.push(`  Overall PASS/FAIL: ${result.data.overallPass === null ? "PENDING" : result.data.overallPass ? "PASS" : "FAIL"}`);
